...
class BalanceMasasEmpaqueController extends Controller
{
    ...
    private function queryProduccionAtribuida(array $validated, Collection $processIds, array $groups): array
    {
        ...
        foreach ($producciones as $produccion) {
            ...
            $procesoPrincipalId = (int) ($produccion->proceso_id ?? 0);
            $folioPrincipal = $procesoToFolio[$procesoPrincipalId] ?? null;
            if ($folioPrincipal) {
                $pesoBascula = (float) ($produccion->peso_bascula_kg ?? 0);
                $pesoNeto = (float) ($produccion->peso_neto_kg ?? 0);
                $pesoVisible = $pesoBascula > 0 ? $pesoBascula : $pesoNeto;

                if ($pesoVisible > 0) {
                    $mainOwnedVisibleByFolio[$folioPrincipal] = (float) ($mainOwnedVisibleByFolio[$folioPrincipal] ?? 0) + $pesoVisible;
                }
            }
        ...
        // Correccion puntual del folio reportado por usuario.
        $folioPuntual = '27-37-60-0002';
        if (isset($mainOwnedVisibleByFolio[$folioPuntual])) {
            $produccionBasculaByFolio[$folioPuntual] = round((float) $mainOwnedVisibleByFolio[$folioPuntual], 2);
        }
     ...
     * Replica el criterio base de reparto por cajas.
     */
    private function getProduccionBasculaAtribuidaPorFolio(ProduccionEmpaque $produccion, array $procesoToFolio): array
    {
        ...
        foreach ($detalles as $detalle) {
            ...
            $atribucion[$folio] = ($atribucion[$folio] ?? 0.0) + (($pesoBascula / $totalCajasPallet) * $cajasDetalle);
        }
